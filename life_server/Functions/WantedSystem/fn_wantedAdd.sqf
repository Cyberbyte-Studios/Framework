...
private _query = format ["SELECT id FROM wanted WHERE id='%1'",_uid];
...
if !(count _queryResult isEqualTo 0) then {
    _query = format ["SELECT crimes, bounty FROM wanted WHERE id='%1'",_uid];
    ...
    _query = format ["UPDATE wanted SET crimes = '%1', bounty = bounty + '%2', active = '1' WHERE id='%3'",_pastCrimes,_val,_uid];
    [_query,1] call DB_fnc_asyncCall;
} else {
    _crime = [_type select 0];
    _crime = [_crime] call DB_fnc_mresArray;
    _query = format ["INSERT INTO wanted (id, name, crimes, bounty, active) VALUES('%1', '%2', '%3', '%4', '1')",_uid,_name,_crime,_val];
    [_query,1] call DB_fnc_asyncCall;
};
